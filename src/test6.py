    ...
    try:
        while True:
            ...
            try:
                ...
                # 색상 보정
                image = ImageEnhance.Color(image).enhance(1.5)  # 색상 강화
                image = ImageEnhance.Brightness(image).enhance(1.2)  # 밝기 조정
                image = ImageEnhance.Contrast(image).enhance(1.2)  # 대비 조정
            ...
            try:
                ...
                boxes, logits, phrases = predict(
                    model=model,
                    image=image_tensor,
                    caption="a person with a knife, fallen person, fork, person running away",
                    box_threshold=0.4,
                    text_threshold=0.35
                )
